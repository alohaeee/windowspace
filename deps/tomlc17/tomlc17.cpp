...
static pool_t *pool_create(int N) {
  ...
  pool_t *pool = (pool_t *)MALLOC(totalsz);
  if (!pool) {
    return NULL;
  }
...
static toml_datum_t *tab_emplace(toml_datum_t *tab, span_t key,
  ...
  {
    char **pkey = (char **)REALLOC(tab->u.tab.key, sizeof(*pkey) * align8(N + 1));
    if (!pkey) {
  ...
  {
    int *plen = (int *)REALLOC(tab->u.tab.len, sizeof(*plen) * align8(N + 1));
    if (!plen) {
      *reason = "out of memory";
      return NULL;
    }
  ...
  {
    toml_datum_t *value =
        (toml_datum_t *)REALLOC(tab->u.tab.value, sizeof(*value) * align8(N + 1));
    if (!value) {
      *reason = "out of memory";
      return NULL;
    }
...
static toml_datum_t *arr_emplace(toml_datum_t *arr, const char **reason) {
  ...
  toml_datum_t *elem = (toml_datum_t *)REALLOC(arr->u.arr.elem, sizeof(*elem) * align8(n + 1));
  if (!elem) {
    *reason = "out of memory";
    return NULL;
  }
...
toml_result_t toml_parse_file(FILE *fp) {
  ...
  while (!feof(fp)) {
    ...
    if (top == max) {
      ...
      char *tmp = (char *)REALLOC(buf, tmpmax + 1);
      if (!tmp) {
...
static int parse_norm(parser_t *pp, token_t tok, span_t *ret_span) {
  ...
  p = (char *)memchr(ret_span->ptr, '\\', ret_span->len);
  if (!p) {
    return 0; // success
  }
